...
from rsl_rl.utils.motion_lib.motion_utils.flags import flags
...
from isaac_utils.isaac_utils.rotations import(
    quat_angle_axis,
    quat_inverse,
    quat_mul_norm,
    get_euler_xyz,
...
class MotionLibBase():
    ...
    def load_data(self, motion_file, min_length=-1, im_eval = False, motion_list=None):
        ...
        if self.mode == MotionlibMode.file:
            if min_length != -1:
                ...
                data_list = self._motion_data_load
            # **Filter by motion_list if provided**
            if motion_list:
                data_list = {k: v for k, v in data_list.items() if k in motion_list}
    ...
    def load_motions(self, 
                 random_sample=True, 
                 start_idx=0,  # ✅ Ensure start_idx is used properly
                 max_len=-1, 
                 target_heading=None,
                 motion_list=None,
                 _motion_data_keys_original=None,
                 _motion_data_list_original=None):
        ...
        self._motion_data_keys_original = _motion_data_keys_original
        self._motion_data_list_original = _motion_data_list_original
        ...
        # ✅ Ensure we use the correct motion list
        print('🔍 Motion list provided:', motion_list)
        
        if motion_list:
            self._motion_data_keys = np.array(self._motion_data_keys_original)
            filtered_indices = [i for i, key in enumerate(self._motion_data_keys_original) if key in motion_list]
            print('✅ Corrected filtered_indices:', filtered_indices)

            self._motion_data_keys = self._motion_data_keys_original[filtered_indices]
            self._motion_data_list = self._motion_data_list_original[filtered_indices]

            num_selected_motions = len(self._motion_data_keys)

            if num_selected_motions == 0:
                raise ValueError("❌ No matching motions found in the selected motion list!")

            print(f"🚀 Strictly filtered motion keys: {self._motion_data_keys}")
        else:
            num_selected_motions = len(self._motion_data_keys)
            filtered_indices = list(range(num_selected_motions))  # <== 加这行，确保下面还能用到

        # ✅ Integrate `start_idx` into motion selection
        if random_sample:
            sample_idxes = torch.randint(0, num_selected_motions, (num_motion_to_load,), device=self._device)
        else:
            sample_idxes = (torch.arange(start_idx, start_idx + num_motion_to_load, device=self._device) % num_selected_motions)

        # ✅ Convert sampled indices to match original dataset
        sample_idxes_original = torch.tensor([filtered_indices[i] for i in sample_idxes.cpu().numpy()], device=self._device)

        self._curr_motion_ids = sample_idxes_original
        self.curr_motion_keys = np.array(self._motion_data_keys)[sample_idxes.cpu().numpy()]

        print(f"🚀 Using start_idx={start_idx}, selected motion keys: {self.curr_motion_keys[:5]}")

        logger.info(f"✅ Loading {num_motion_to_load} motions...")
        logger.info(f"🔢 Sampling motion indices: {sample_idxes[:5]}, ....")
        logger.info(f"📌 Current motion keys: {self.curr_motion_keys[:5]}, ....")

        motion_data_list = self._motion_data_list_original[sample_idxes_original.cpu().numpy()]
        print('📌 motion_data_list.shape:', motion_data_list.shape)
